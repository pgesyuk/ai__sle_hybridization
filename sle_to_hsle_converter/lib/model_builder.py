...
def _dereference_file_symlinks(root: str) -> None:
    """Replace file-symlinks with writable copies of their targets.

    shutil.copytree(symlinks=True) preserves symlinks verbatim.  If a symlink
    points to a file in a shared read-only area, any build script that opens it
    for writing will follow the link and fail with PermissionError.

    This function walks *root* and, for every symlink whose resolved target is a
    regular file, removes the symlink and replaces it with an actual copy of the
    target (mode u+w).  Symlinks that resolve to directories, and dangling
    symlinks (target does not exist), are left untouched.
    """
