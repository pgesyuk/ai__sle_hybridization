...
def three_way_merge(
    ...
    *,
    ...
    output_rel_path: if provided, use this path under output/ instead of
                     entry.rel_path (used for path remapping). Source paths
                     (ref_sle, ref_hsle) always use entry.rel_path.
    ...
    current = current_path or os.path.join(output, out_path)
