...
# Note: filelists/sip.list is intentionally NOT listed here — it is a
# project configuration file (specifying the pre-built PCD location) that
# lives in the SLE source tree and must be preserved as-is so the HSLE
# build finds the correct pchlp.  Only the filelists_rtl-generated vip
# files are true build artifacts that should be absent from source.
...
def create_output(new_sle: str, output: str) -> None:
    ...
    Key principle: filelists/ content is derived entirely from the INPUT SLE
    model (preserved by the base copy).  filelists/.sip.list.mako and
    filelists/.soc.list.mako carry the project-specific version references
    (pkg_pinlist release, hub version) for the SLE being hybridized.
    filelists/sip.list carries the pre-built PCD location — it is a project
    configuration file, NOT a build artifact, and must be preserved so the
    HSLE build finds the correct pchlp.  Only filelists_rtl-generated vip
    files are removed (see _COMPILE_OUTPUT_FILES).
